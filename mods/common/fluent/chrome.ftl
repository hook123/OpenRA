...
label-assetbrowser-source-selector-desc = 选择素材来源
dropdownbutton-assetbrowser-source-selector = 文件夹
dropdownbutton-assetbrowser-asset-types-dropdown = 资源类型
label-assetbrowser-filename-desc = 按名称过滤
label-assetbrowser-sprite-scale = 比例:
label-assetbrowser-palette-desc = 调色板:
label-assetbrowser-sprite-bg-error = 无法显示文件。详细信息请查看 assetbrowser.log。
...
button-connectionfailed-panel-abort = 终止
label-connecting-panel-title = 连接中
button-connecting-panel-abort = 终止
...
button-connection-switchmod-panel-abort = 终止
...
button-map-generator-generate-random = 生成随机
...
button-select-categories-buttons-all = 全局
...
button-gamesave-browser-panel-load = 加载
...
image-lobby-servers-bin-requires-authentication-tooltip = 需要OpenRA论坛账号
dropdownbutton-lobby-servers-bin-filters = 过滤游戏

## lobby-servers.yaml, multiplayer-browser.yaml
label-container-server = 服务器
label-container-players = 玩家数
label-container-location = 位置
label-container-status = 状态
label-notice-container-outdated-version = 您运行的OpenRA版本过时。请从www.openra.net下载最新版本
label-notice-container-unknown-version = 您运行的OpenRA版本无法识别。请从www.openra.net下载最新版本
label-notice-container-playtest-available = OpenRA的下一个版本预览版可供测试。请从www.openra.net下载测试版

## lobby.yaml
dropdownbutton-server-lobby-slots = 槽位管理
button-server-lobby-reset-options = 重置默认
button-skirmish-tabs-players-tab = 玩家数
button-skirmish-tabs-options-tab = 选项
button-multiplayer-tabs-players-tab = 玩家数
button-multiplayer-tabs-options-tab = 选项
button-multiplayer-tabs-servers-tab = 服务器
button-server-lobby-changemap = 更改地图
...
label-mainmenu-introduction-prompt-desc-a = 欢迎回来指挥官！使用下面的选项初始化战斗参数。
label-mainmenu-introduction-prompt-desc-b = 您可以在设置菜单中稍后配置其他选项。
label-mainmenu-system-info-prompt-title = 建立战场控制
label-mainmenu-system-info-prompt-text-a = 我们希望收集一些系统细节，以帮助我们优化OpenRA。
label-mainmenu-system-info-prompt-text-b = 在您的许可下，以下匿名数据将在每次游戏启动时发送：
...
button-mapchooser-panel-delete-all-maps = 删除所有地图
...
label-mapchooser-random-map-tileset = 图块集:
...
label-missionbrowser-normal-difficulty = 正常

## multiplayer-browser.yaml
image-multiplayer-panel-password-protected-tooltip = 需要密码
image-multiplayer-panel-requires-authentication-tooltip = 需要OpenRA论坛账号
button-selected-server-join = 加入
dropdownbutton-multiplayer-panel-filters = 过滤游戏
button-multiplayer-panel-directconnect = 直接IP
...
checkbox-multiplayer-createserver-panel-advertise = 在线广告
...
dropdownbutton-video-mode-dropdown-container = 窗口模式
